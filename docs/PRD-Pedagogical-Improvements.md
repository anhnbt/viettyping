...
- [x] **Module Đo tốc độ & Time Attack (`TypingSpeedController`)**:
...
- [x] **Module Toán thực tế kéo thả (`RealWorldMathGame`)**:
...
- [x] **Module Canvas Tô màu (`ColoringCanvas`)**:
...
- [x] **Giao diện Tiêu điểm & Quản lý Pomodoro (`FocusModeLayout` & `FocusGardenTimer`)**:
