...
const TopicSelector: React.FC<TopicSelectorProps> = ({ subject, onSelectTopic, onBack }) => {
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="text-center flex-1">
          <div className={`inline-flex items-center gap-3 px-6 py-3 rounded-full bg-gradient-to-r ${subject.color} text-white mb-2`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-green-50 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-green-600">
            {subject.topics.reduce((acc, topic) => acc + topic.activities.length, 0)}
          </div>
          <div className="text-sm text-green-800">Hoạt động</div>
        </div>
        <div className="bg-purple-50 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-purple-600">
            {Math.round(subject.topics.reduce((acc, topic) => acc + topic.estimatedTime, 0) / subject.topics.length)}
          </div>
          ...
          >
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className={`w-8 h-8 rounded-full bg-gradient-to-r ${subject.color} text-white flex items-center justify-center text-sm font-bold`}>
                  {index + 1}
                </div>
                <div className={`px-2 py-1 rounded-full text-xs font-medium ${getDifficultyColor(topic.difficulty)}`}>
              ...
              <div className="mt-4 flex flex-wrap gap-1">
                {[...new Set(topic.activities.map(activity => activity.type))].map(type => (
                  <span
                    key={type}
...
const getActivityTypeText = (type: string) => {
  switch (type) {
    case 'typing': return 'Gõ phím';
    case 'quiz': return 'Trắc nghiệm';
    case 'drawing': return 'Vẽ';
